...
def choose_date():
    ...
    # time.sleep(2)
    # 選擇日期
    button1 = driver.find_element_by_link_text("4")
    button1.click()
    time.sleep(2)
...
def option_callitem_crawling():
    ...
    # 買權
        for i in range(o, o+1):
            ...
            a = driver.find_element_by_xpath(
                '//*[@id="printhere"]/div[3]/table[1]/tbody/tr[1]/td/table/tbody/tr/th')
            call = a.text
            temp.append(call)
    # 項目欄位
            for j in range(i, i+1):
                b = driver.find_elements_by_css_selector("tr[bgcolor]")[j]
    # 到期月份
                for k in range(j, j+1):
                    c = driver.find_elements_by_css_selector(
                        "tr[bgcolor] td[width='48'][align='middle']")[k]
                    expire = c.text
                    temp.append(expire)
    # 履約價格: Strike Price
    ...
    # 未平倉量: Open Interest
                        for m in range(l, l+1):
                            e = driver.find_elements_by_css_selector("td[width='66']")[
                                m]
                            oi = e.text
                            temp.append(oi)
                            box.append(temp)
